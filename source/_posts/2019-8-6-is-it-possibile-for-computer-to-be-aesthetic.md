...
这篇文章采用 multi-task 的方法，除了让网络回归 ground truth 的标注分数 (这里包括多个维度的特征) 外，还让网络学习出不同图片之间的评分差异 (即 rank loss)。同时，在 rank loss 上研究了不同人员标注的一致性，有一定的参考意义。
